...
load_tagging_data = function(filename){
  ...
  tagging_data = read.csv(filename, stringsAsFactors = FALSE)
  ### Adjusting Column names
      colnames(tagging_data)[1]  <- 'unique_id'
      colnames(tagging_data)[2]  <- 'datetime'
      colnames(tagging_data)[3]  <- 'species'
      colnames(tagging_data)[4]  <- 'conventional_tag_id'
      colnames(tagging_data)[5]  <- 'vem_tag_type'
      colnames(tagging_data)[6]  <- 'vem_tag_serial'
      colnames(tagging_data)[7]  <- 'vem_tag_id'
      colnames(tagging_data)[8]  <- 'fork_length(cm)'
      colnames(tagging_data)[9]  <- 'precaudal_length(cm)'
      colnames(tagging_data)[10] <- 'cohort'
      colnames(tagging_data)[11] <- 'area_of_capture'
      colnames(tagging_data)[12] <- 'depth_of_capture'
      colnames(tagging_data)[13] <- 'lat_deg'
      colnames(tagging_data)[14] <- 'lat_min'
      colnames(tagging_data)[15] <- 'lon_deg'
      colnames(tagging_data)[16] <- 'lon_min'
      colnames(tagging_data)[17] <- 'lat'
      colnames(tagging_data)[18] <- 'lon'
      colnames(tagging_data)[19] <- 'stomach_everted'
      colnames(tagging_data)[20] <- 'eyes_popped'
      colnames(tagging_data)[21] <- 'bladder_vented'
      colnames(tagging_data)[22] <- 'point_of_incision'
      colnames(tagging_data)[23] <- 'dna_clip'
      colnames(tagging_data)[24] <- 'cannulation'
      colnames(tagging_data)[25] <- 'sex'
      colnames(tagging_data)[26] <- 'video'
      colnames(tagging_data)[27] <- 'photo'
      colnames(tagging_data)[28] <- 'photo_name'
      colnames(tagging_data)[29] <- 'audio_log_file'
      colnames(tagging_data)[30] <- 'dropshot'
      colnames(tagging_data)[31] <- 'tissue_sample'
      colnames(tagging_data)[32] <- 'gut_sample'
      colnames(tagging_data)[33] <- 'tagger'
      colnames(tagging_data)[34] <- 'notes'
      colnames(tagging_data)[35] <- 'recaptured'
      colnames(tagging_data)[36] <- 'detections'
      colnames(tagging_data)[37] <- 'comments'
    #### Converting tagging date to POSIX object
      tagging_data$datetime = as.POSIXct(tagging_data$datetime, format = '%m/%d/%y %H:%M', tz = 'HST')
...
remove_location_from_vue = function(vue_df, location_to_remove = FALSE){
  #### Removes station(s) provided by location_to_remove argument from vue data
    vue_df = vue_data[-(which(vue_data$station %in% location_to_remove)), ]
}
...
cluster_receivers = function(receiver_data, n_clusters = 5, lon = TRUE){
  #### Groups receivers into clusters based on their spatial relationships. 
   ### If lon = FALSE, cluster only by north south, if TRUE, also east west. 
  if(lon == FALSE){
...
get_graph = function(vue_df, 
  ...
  ### with these stations. 
  if (tag_ids[1] == FALSE){ # if no tag ids specified, all tag ids used
  ...
  vue_df = clean_vue(vue_df, tag_id, exclude = FALSE)
  ...
  adj_matrix = matrix(data = 0, nrow = numstations, ncol = numstations)
...
generate_tagging_detection = function(tagging_data, vue_data){
  ...
  for (i in 1:length(tag_ids)){
    ...
    false_record$datetime  = as.POSIXct(tagging_data$datetime[which(tagging_data$vem_tag_id == tag_ids[i])])
    ...
    false_record$lat  = convert_lat_lon(tagging_data$lat_deg[which(tagging_data$vem_tag_id == tag_ids[i])], tagging_data$lat_min[which(tagging_data$vem_tag_id == tag_ids[i])])
    false_record$lon = convert_lat_lon(tagging_data$lon_deg[which(tagging_data$vem_tag_id == tag_ids[i])], tagging_data$lon_min[which(tagging_data$vem_tag_id == tag_ids[i])])
    false_record$full_tag_id = vue_data$full_tag_id[vue_data$tag_id == tag_ids[i]][1]
...
generate_study_date = function(vue_df, receiver_df = NULL, start_date = FALSE){
...
generate_receiver_study_date = function(receiver_df, vue_df = NULL, start_date = FALSE){
  ...
  if(start_date == FALSE){
    if(is.null(vue_df$study_date)){
      start_date = min(receiver_df$deployment_date)
    }else{
      start_date = min(vue_df$datetime) - min(vue_df$study_date)
    }
  }
  receiver_df$deployment_study_date = as.numeric(difftime(receiver_df$deployment_date, as.POSIXct(start_date), units = 'days'))
  receiver_df$recovery_study_date = as.numeric(difftime(receiver_df$recovery_date, as.POSIXct(start_date), units = 'days'))
  return(receiver_df)
}
...
tag_detection_histogram = function(vue_data, receiver_data = NULL){
  ### Revised function for plotting daily histogram of the number of detections.
  ## If receiver_data argument is not null, plot lines over histogram showing when receivers were in and out of water
  if(is.null(receiver_data) == FALSE){
    ### Getting dates when receivers in the study were rearranged.
    receiver_data_subset = receiver_data[receiver_data$station_name %in% unique(vue_data$station), ] # First subset only stations where a fish was detected
    receiver_study_dates = unique(c(floor(receiver_data_subset$deployment_study_date), floor(receiver_data_subset$recovery_study_date))) # Then pull out unique dates on which those stations were serviced
    receiver_study_dates = receiver_study_dates[is.na(receiver_study_dates) == FALSE]
    receiver_dates = receiver_study_dates[which(receiver_study_dates >= floor(min(vue_data$study_date)) & receiver_study_dates <= ceiling(max(vue_data$study_date)))]
  }
  ...
  for(i in 1:length(unique(vue_data$tag_id))){
    ...
    hist(floor(indv_data$study_date), right = TRUE, breaks = 0:ceiling(max(vue_data$study_date)),
         ...
         col = 'black',
         xaxt = 'n') # fills in histogram bars
    date_labels = as.character(strptime(seq(min(vue_data$datetime), max(vue_data$datetime), length.out = 10), format = '%Y-%m-%d'))
    axis(side = 1, at = seq(min(vue_data$study_date), max(vue_data$study_date), length.out = 10), labels = date_labels, las = 2, cex = .25)
...
plot_receiver_map = function(receiver_data, filename = FALSE, region = 'Makapuu', rec_col = FALSE){
  ...
  ## Plotting basemap
  plot.bathy(bathymetry, land = TRUE, image=TRUE, bpal = list(c(-400, -1, "lightskyblue")), deepest.isobath = c(-10000), shallowest.isobath = c(0), step = c(100), drawlabels = TRUE)
  ## Adding scale legend
...
plot_movements = function(vue_data, receiver_data, region = 'Makapuu', tag_ids = FALSE, rec_col = FALSE, plot_title = FALSE){
  ...
  for (i in 1:length(unique(vue_data$tag_id))){
    ...
    if(plot_title == FALSE){
      plot_title = paste(as.character(unique(vue_data$tag_id)[i]), 'Movement Map.png')
    }
    png(plot_title)
    ...
    if(class(rec_col) == 'list'){
      ...
      for(r in 1:length(receiver_data_to_plot$station_name)){
        if(receiver_data_to_plot$station_name[r] %in% color_palette$station){
          receiver_plot_colors[r] = color_palette$colors[which(color_palette$station == receiver_data_to_plot$station_name[r])]
        }
      }
    }
...
plot_tag_detections = function(vue_data, receiver_data, start_date = FALSE, end_date = FALSE, date_format = "%Y-%m-%d %H:%M:%S", plot_lost = FALSE, remove_station = FALSE, region = "Makapuu", tag_ids = FALSE, rec_col = FALSE, title = FALSE, plot_receivers = "all"){
...
plot_mortality_stripchart  = function(analysis_summary, valid_tags, questionable_tags, expired_tags){
  png('tag mortality chart.png', width = 1600, height = 1000)
  ...
  questionable_tags = questionable_tracks[questionable_tracks %in% unique(analysis_summary$data$tag_id)]
  ...
  axis(side = 1, at = seq(min(analysis_summary$data$datetime), max(analysis_summary$data$datetime), length.out = 10), labels = date_labels, las = 2, cex = .25)
  dev.off()
}
...
run_analysis = function(vue_data, tagging_data, start_date = FALSE, end_date = FALSE){
  ...
  receiver_data = generate_receiver_study_date(receiver_df = receiver_data, vue_df = vue_df)
  ...
  analysis_summary$tagging_date = get_tagging_date(vue_data = vue_df, tagging_data = tagging_data)
  analysis_summary$fork_length = get_fork_length(vue_data = vue_df, tagging_data = tagging_data)
...
create_analysis_csv = function(analysis_summary, tagging_data = tagging_data, start_date, end_date){
  # analysis_summary = run_analysis(vue_data = vue_data, tagging_data = tagging_data, start_date = start_date, end_date = end_date)
  tag_id = as.vector(analysis_summary$tag_id)
  tagging_date = analysis_summary$tagging_date
  fork_length = analysis_summary$fork_length
  time_at_liberty = analysis_summary$time_at_liberty
  n_detections = analysis_summary$n_detections
  percent_of_detections = analysis_summary$n_detections/sum(analysis_summary$n_detections)
  detections_per_day = n_detections/time_at_liberty
  unique_days_detected = analysis_summary$unique_days_detected
  distance_tracked = analysis_summary$track_distance
  distance_per_day = distance_tracked / time_at_liberty
  receivers_detected = analysis_summary$n_stations_by_tag
  movements = analysis_summary$movements
  movements_by_day = movements/time_at_liberty
  brfa_crossings = rowSums(analysis_summary$brfa_stats[, 1:2])
  time_tracked_in = analysis_summary$brfa_stats[ ,3]
  time_tracked_out = analysis_summary$brfa_stats[ ,4]
  brfa_crossings_per_day = brfa_crossings/time_at_liberty
  homerange_polygon = analysis_summary$homerange[ ,1]
  homerange_linear = analysis_summary$homerange[ ,2]
  analysis_df = data.frame(cbind(tag_id, tagging_date, fork_length, time_at_liberty,n_detections, percent_of_detections, detections_per_day, unique_days_detected, distance_tracked, distance_per_day, receivers_detected, movements, movements_by_day, brfa_crossings, time_tracked_in, time_tracked_out, brfa_crossings_per_day, homerange_polygon, homerange_linear))
  colnames(analysis_df) = c('tag_id', 'tagging_date', 'fork_length', 'time_at_liberty','n_detections', 'percent_of_detections', 'detections_per_day', 'unique_days_detected', 'distance_tracked', 'distance_per_day', 'receivers_detected', 'movements', 'movements_by_day', 'brfa_crossings', 'time_tracked_in', 'time_tracked_out', 'brfa_crossings_per_day', 'homerange_polygon', 'homerange_linear')
...
create_day_night_plot = function(vue_data, receiver_data = NULL, color_palette = FALSE, start_date = FALSE, end_date = FALSE, date_format = "%Y-%m-%d %H:%M:%S"){
  ### Function to produce day night plots modified from code provided by Alex Filous with individual days on X axis and time of day on Y axis. Shading represents night/day
    ## If provided, receiver_data argument will take deployment and recovery dates and score plot vertically with a bar on dates that equipment was serviced
  if(start_date == FALSE){
    start_date = min(vue_data$datetime)
  ...
  ### If receiver_data argument is present, pull relevant dates for plotting vetical bars
  if(is.null(receiver_data) == FALSE){
    ### Getting dates when receivers in the study were rearranged.
    receiver_data_subset = receiver_data[receiver_data$station_name %in% unique(vue_data$station), ] # First subset only stations where a fish was detected
    receiver_dates = unique(c(trunc(receiver_data_subset$deployment_date, 'day'), trunc(receiver_data_subset$recovery_date, 'day'))) # Then pull out unique dates on which those stations were serviced
    receiver_dates = receiver_dates[which(receiver_dates >= floor_date(min(vue_data$datetime)) & receiver_dates <= ceiling_date(max(vue_data$datetime)))]
  }
  ...
  for(i in 1:length(unique(vue_data$tag_id))){
    ...
    ### Subsetting data
    indv_data = vue_data[vue_data$tag_id == unique(vue_data$tag_id)[i],]
    with(vue_data, plot(plot_date,plot_time,col=plot_color,main=unique(vue_data$tag_id)[i],pch=19,cex=1,ylim=c(0,24),type="n", xaxt = 'n',xaxs="i",yaxs="i",xlab="", ylab="Time"))
    date_labels = as.character(strptime(seq(min(vue_data$datetime), max(vue_data$datetime), length.out = 10), format = '%Y-%m-%d'))
    axis(side = 1, at = seq(min(vue_data$plot_date), max(vue_data$plot_date), length.out = 10), labels = date_labels, las = 2, cex = .25)
    #if (nrow(indv_data)>1){
...
plot_path_use = function(movement_graph, vue_df, region = 'Makapuu'){
  ...
  for(r in 1:length(unique_stations)){
    for(c in 1:length(unique_stations)){
      ...
      lines(lat~lon, data = move_line, col = colors[color_scaled_graph[r,c]], lwd = 2)
    }
  }
  dev.off()
}
...
get_bathymetry = function(region){
if(region == 'Makapuu'){
  bathymetry = getNOAA.bathy(lon1 = -157.8, 
                             lon2 = -157.5, 
                             lat1 = 21.2, 
...
#### Getting the size of each brfa by polygon sizeusing their coordinates
get_brfa_areas = function(){
  brfa_areas = c()
  # #### Size of BRFAs
  # # BRFA A
...
#### Make Test Data
make_test_data = function(vue_data){
  ### Selecting 6 tags for quick testing purposes and returning their data.
  test_tags = c(vue_data$tag_id[1], vue_data$tag_id[length(vue_data$tag_id)], 18260, 18259, 57358, 57457)
  test_data = vue_data[vue_data$tag_id %in% test_tags, ]
  return(test_data)
}

####################### DISSERTATION HYPOTHESES TESTING #################
dissertation_H1 = function(analysis_summary){
...
dissertation_H2 = function(analysis_summary){
  ...
  n_tags_with_3_plus_positions = length(which(analysis_summary$homerange[ ,1] != 0))
  ## Area for tags with 3 or more positions from max polygon (km^2) - Mean and Standard Error
  print(paste("There are",  n_tags_with_3_plus_positions, "tags with 3 or more known positions to compare to BRFA areas using max polygon. They have a mean area of", round(mean(n_tags_with_3_plus_positions), digits = 2), 'km^2 with a standard error of',  round(std_error(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])[1], digits = 2)))
  ## Area for tags with 3 or more positions from max polygon (km^2) - Median, Min, 1st Quantile, 3rd Quantile, Max
  print(paste("They have a median area of" , round(fivenum(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])[3], digits = 2), "(min =", round(fivenum(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])[1], digits = 2), ", first quantile =", round(fivenum(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])[2], digits = 2), ", third quantile =", round(fivenum(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])[4], digits = 2), ", max =", round(fivenum(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])[5], digits = 2), ")"))
  ## Square Rooted Area for tags wtih 3 or more positions from max polygon (km^2) - Mean and Standard Error
  print(paste("Their mean square-rooted area is", round(mean(sqrt(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])), digits = 2), ", standard error =", round(std_error(sqrt(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1])), digits = 2)))
  ## Square Rooted Area for tags wtih 3 or more positions from max polygon (km^2) - Median, Min, 1st Quantile, 3rd Quantile, Max
  print(paste("They have a median sqrted length of" , round(fivenum(sqrt(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1]))[3], digits = 2), "(min =", round(fivenum(sqrt(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1]))[1], digits = 2), ", first quantile =", round(fivenum(sqrt(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1]))[2], digits = 2), ", third quantile =", round(fivenum(sqrt(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1]))[4], digits = 2), ", max =", round(fivenum(sqrt(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0, 1]))[5], digits = 2), ")"))
  
  ## Maximum Linear Distance for all tags - Mean, Standard Error
  print(paste("The mean linear distance for all", length(analysis_summary$homerange[,2]), "tags was", round(mean(analysis_summary$homerange[,2]), digits = 2), "km. Standard error =", round(std_error(analysis_summary$homerange[,2]), digits = 2)))
  ## Maximum Linear Distance for all tags - Median, Min, 1st Quantile, 3rd 
  print(paste("The median linear distance for the", length(analysis_summary$homerange[,2]), "tags was", round(fivenum(analysis_summary$homerange[, 2])[3], digits = 2), "(Min =", round(fivenum(analysis_summary$homerange[ ,2])[1], digits = 2), "First quantile =", round(fivenum(analysis_summary$homerange[, 2])[2], digits = 2), "Third Quantile =", round(fivenum(analysis_summary$homerange[, 2])[4], digits = 2), "Max =", round(fivenum(analysis_summary$homerange[, 2])[5], digits = 2), ")"))
  ...
  if(exists('fig.dir')){
    ...
    h2_areas_sqr = rbind(cbind(brfa_areas_km2, "BRFA"), cbind(analysis_summary$homerange[analysis_summary$homerange[ ,1] != 0 ,1], "P. filamentosus"))
    colnames(h2_areas_sqr) = c("area", "source")
    rownames(h2_areas_sqr) = NULL
    png(filename = "H2 - Boxplot - Homerange vs BRFA Area KM2.png")
    boxplot(as.numeric(h2_areas_sqr[, 'area']) ~ as.factor(h2_areas_sqr[ ,'source']), main = "Max Polygon Approach to Homerange vs. BRFA Area", ylab = "km^2")
    dev.off()
    
    ### Boxplot Comparing linear distance
    ## Creating a dataframe to store linear distances for BRFAs and Fish
    h2_areas_linear = rbind(cbind(sqrt(brfa_areas_km2), "BRFA"), cbind(sqrt(analysis_summary$homerange[ ,2]), "P. filamentosus"))
    colnames(h2_areas_linear) = c("area", "source")
    rownames(h2_areas_linear) = NULL
    png(filename = "H2 - Max Linear Homerange vs. sqrt BRFA Area.png")
    boxplot(as.numeric(h2_areas_linear[, 'area']) ~ as.factor(h2_areas_linear[ ,'source']), main = "Max Linear Homerange vs. sqrt(BRFA Area)", ylab = "km")
    dev.off()
  }
}
...
####### END DISSERTATION ANALYSIS FUNCTIONS ########
...
vue_data = load_vemco(filename = file.path(data_dir, 'VUE_Export_2017_June_25.csv'))
...
tag_specs = read.csv('/Users/stephenscherrer/Google Drive/Weng Lab/Data/VEMCO/VEMCO Tag Specifications.csv', stringsAsFactors = FALSE)
...
vue_data = vue_data[vue_data$full_tag_id %in% tag_specs$vue_tag_id, ]
...
# 
 View(receiver_data[c((dim(receiver_data)[1]-44 ): dim(receiver_data)[1]), ] )
...
# setwd(analysis_summary$fig_dir)
...
run = function(vue_data, tagging_data, receiver_data, start_date = FALSE, end_date = FALSE, tag_ids = FALSE, filter_false_detections = FALSE, run_description = FALSE, region = "Makapuu", valid_tracks = NULL, questionable_tracks = NULL, expired_tracks = NULL, plot = TRUE, report = TRUE){
  ...
  ##### Filtering for false detections
  #### False detection filtering is now handled by the UHHPC due to run constraints. 
  # if(filter_false_detections == TRUE){
  #   vue_df$false_detection_index = remove_false_detections(vue_data, remove_detections = FALSE) # Removing false detections
  # }else{
  #   vue_df$false_detection_index = TRUE
  # }
  
  ############ Analysis #################
  ### Creating a adjacency matrix graph of all tag movements. Shows movement from one station to another
    analysis_summary$movement_graph = get_graph(vue_df = vue_df, start_date = start_date, end_date = end_date, igraph = FALSE)
  ### Creating a matrix populated by the distance in km between each station in space
    analysis_summary$receiver_distance_graph = distance_between_vue_receivers(vue_df = vue_df)
  ...
  ### Creating a "detection" for when and where the animal was tagged
    vue_df = generate_tagging_detection(tagging_data = tagging_data, vue_data = vue_df)
  ...
  ### Creating study date, the number of days relative to the start of  the study - adjusts all date times relative to start of study
    vue_df = generate_study_date(vue_df) 
    analysis_summary$receiver_data = generate_receiver_study_date(receiver_df = receiver_data, vue_df = vue_df)
  ### Making detection matrix - Each row is a Tag ID, each station in a column. 
    ### matrix values are number of transmissions for a tag at a given station
      ## Note: Excludes tagging location
    analysis_summary$tag_detection_matrix = build_detection_matrix(vue_df[vue_df$station != 'Tagging Location', ])  
  
  ### Determining how many times a single tag was detected
  ## Note: Excludes tagging location
    analysis_summary$n_detections_by_tag = rowSums(analysis_summary$tag_detection_matrix)  
  ## How many tags were detected at 2 or more stations?
  ## Note: Excludes tagging location
    analysis_summary$n_stations_by_tag = rowSums(analysis_summary$tag_detection_matrix / analysis_summary$tag_detection_matrix, na.rm = TRUE)  
  ## How many unique days were tags detected?
    ## Note: Excludes tagging
    analysis_summary$unique_days_detected = calculate_days_detected(vue_df[vue_df$station != 'Tagging Location', ])  
  ## How many days was an individual tracked?
    analysis_summary$time_at_liberty = calculate_time_at_liberty(vue_df)  
  ## How many transmissions were detected per day on average?
    analysis_summary$transmissions_per_day = list()
    # for entire time at liberty
      analysis_summary$transmissions_per_day$at_liberty = analysis_summary$n_detections_by_tag / analysis_summary$time_at_liberty
    # only for days a tag was detected
      analysis_summary$transmissions_per_day$days_detected = analysis_summary$n_detections_by_tag / analysis_summary$unique_days_detected  
    ## How many days after tracking until individual appears on array?
      analysis_summary$days_before_detection = calculate_days_before_detection(vue_df)  
    ## What percentage of days at liberty was a tag detected?
      analysis_summary$percentage_of_days_at_liberty_detected = calculate_days_detected(vue_df) / ceiling(calculate_time_at_liberty(vue_df))
  ## How far was a tag tracked?
    analysis_summary$track_distance = distance_tracked(vue_df)
  ## How many times was a tag detected changing station?
    analysis_summary$movements = n_movements(vue_df)
  ## What stations was a tag detected on?
    analysis_summary$detected_stations = list_stations_detected(vue_df)
    #### Movements into / out of BRFA for each fish
    analysis_summary$brfa_stats = brfa_movements(vue_df)
  #### Standardized movements
    analysis_summary$brfa_movements_standardized_by_time_at_liberty = brfa_movements_by_time_at_liberty(analysis_summary$brfa_stats, analysis_summary$time_at_liberty)
  #### Number of fish tagged greater than L50 
    analysis_summary$n_mature = length(which(as.numeric(as.character(tagging_data$"fork_length(cm)"[tagging_data$vem_tag_id %in% unique(vue_df$tag_id)])) >= 43))
    analysis_summary$proportion_mature = round(analysis_summary$n_mature / length(unique(vue_df$tag_id)), digits = 2)
  #### Homerange size
    analysis_summary$homerange = max_movement(vue_df)
  ...
  analysis_summary$h1_results = dissertation_H1(analysis_summary)
  ...
  ## Getting Plotting Bathemytry 
  bathymetry = get_bathymetry(region)
  ### For Makapuu
  # bathymetry = as.bathy(read.table("/Users/stephenscherrer/Google Drive/Weng Lab/Personal_Folders/Steve/dissertation work/Opakapaka Tagging/Opakapaka Tagging Analysis/data/Bathymetry Files/Makapuu Bathymetry"))
  ### For Oahu
  # bathymetry = as.bathy(read.table("/Users/stephenscherrer/Google Drive/Weng Lab/Personal_Folders/Steve/dissertation work/Opakapaka Tagging/Opakapaka Tagging Analysis/data/Bathymetry Files/Oahu Bathymetry"))
  ### For Oahu and Penguin Banks
  # bathymetry = as.bathy(read.table("/Users/stephenscherrer/Google Drive/Weng Lab/Personal_Folders/Steve/dissertation work/Opakapaka Tagging/Opakapaka Tagging Analysis/data/Bathymetry Files/Oahu and Penguin Banks Bathymetry"))
  ...
  if(is.null(valid_tracks) == FALSE & is.null(questionable_tracks) == FALSE & is.null(expired_tracks) == FALSE){
    plot_mortality_stripchart(analysis_summary, valid_tags = valid_tracks, questionable_tags = questionable_tracks, expired_tags = expired_tracks)
  }
  
  # make_detection_plot(analysis_summary$data) ### Dev.off()???
  ### Plotting movements of each tag tags
  plot_tag_detections(analysis_summary$data, receiver_data, start_date = start_date, end_date = end_date, date_format = "%Y-%m-%d %H:%M:%S", plot_lost = FALSE, remove_station = FALSE, tag_ids = FALSE, rec_col = FALSE, plot_receivers = "study", region = region)
  ## plotting relative movements of all fish
  plot_path_use(movement_graph = analysis_summary$movement_graph, vue_df = vue_df, region = region)
  ...
  create_day_night_plot(vue_data = analysis_summary$data, receiver_data = analysis_summary$receiver_data, color_palette = station_palette) # Note: Omitting the receiver_data argument will produce plots without the blue horizontal bars
  #### Detection histogram for each tag 
  tag_detection_histogram(vue_data = analysis_summary$data, receiver_data = analysis_summary$receiver_data) # Note: Ommitting the receiver_data argument will produce plots without the blue horizontal bars
  ...
  ############# Summary table .csv for pooled tag data ###########
  create_analysis_csv(analysis_summary = analysis_summary, tagging_data = tagging_data, start_date = start_date, end_date = end_date)
  ...
  analysis_summary$fig_directory = fig.dir
  analysis_summary$report_directory = report.dir
  
  save.image(file = paste("workspace", run_description, sep = " "))
  setwd(project_dir) # resetting to main project directory 
  
  ################## Reporting Success #############
  print('SUCCESS!')
  beep(8) # SUCCESS!
  send_push(user = 'uGEHvA4hr37tsrCCtpSv4sUUxVuTqN', message = paste("Run", run_description, "Complete!"))
  return(analysis_summary)
}
...
#### Running data from 2012-2015 (Phase 1)
phase1 = run(description = "Phase I - All Receivers",
             vue_data = vue_data, receiver_data = receiver_data, tagging_data = tagging_data,
             start_date = as.POSIXct("2012-04-13"), 
             end_date = as.POSIXct("2014-12-06"),
             region = "Oahu and Penguin Banks")
...
plot_path_use(movement_graph = analysis_summary$movement_graph, vue_df = vue_df, region = 'Makapuu')
...
receiver_df = receiver_data
 start_date = as.POSIXct("2015-03-01", format = "%Y-%m-%d")
 end_date = FALSE
tag_ids = FALSE
filter_false_detections = FALSE
 run_description = "pre_cluster_fdf_testing"
 region = "Makapuu"
 valid_tracks = NULL
