...
load("/Users/stephenscherrer/Documents/Work/UH/Projects/dissertation work/Opakapaka Tagging/Opakapaka Tagging Analysis/workspace_image_updated")

start_date = "2016/1/10"
end_date = "2016/03/6"

vd_data = filter(vue_data, by = datetime >= as.POSIXct(start_date) & datetime < as.POSIXct(end_date)) # filtering fish by study dates
for(i in 1:length(unique(vd_data$tag_id))){
  indv_data = filter(vd_data, by = tag_id == unique(vd_data$tag_id)[i]) # filtering fish based on tag id
  
  ## 1.3.2 creating xy dataframe
...
transmission_interval = 60                      # the interval between subsequent transmissions from a tag
dt = 60         # seconds                       # time-step in the simulation of the trajectory considered (seconds)
P = 60*24          # minutes                       # time-step for pooling acoustic detections (the number of beeps is summed over periods of P*dt seconds) (minutes)
days = as.numeric(difftime(time1 = as.Date(end_date), time2 = as.Date(start_date), units = 'days'))      # Days                          # length of the time series (in days)
detmax = round(P*dt/transmission_interval)      # number of beeps emitted in P*dt seconds (1 beep/minute)
activity_hours = 24                             # number of hours a fish is active and can be detected
# 1.2 Movement parameters corresponding to a home range movement ([1])
# Center of the home range
HRx = xy[1, 1]                                # Longitude (meters)
HRy = xy[1, 2]                                # Latitude (meters)
...
# xy[1,2] = HRy[1]
iterb = 1*activity_hours*60*60/dt                  # number of time steps in one day (14 hours of day-time)
...
iter = days*activity_hours*60*60/dt                # number of time steps in one day (14 hours of day-time)
...
temp=seq(as.Date(start_date), as.Date(end_date), 1)
day=rep(temp,each=activity_hours*60*60/dt)

#### HERE WE HAVE A PROBLEM xy and day != same lenght
XY<-data.frame(xy,day)
colnames(XY) = c("x","y","Day")

# Averaged position at each period of P time steps
iterP = ceiling(iter/P)                                    	# length of the series
...
SUR = as.data.frame(GPS.Points.rec.UTM.Spatial.Data)
  ## Note: Class should be data.frame, values should be class numeric
rx = SUR[,1]
ry = SUR[,2]
colnames(SUR) = c("x", "y")
R=length(rx)                                   # number of receivers

#### End Implementing my SUR array ####


# 2.2 Plot the acoustic array and the trajectory (allplots are print in point 5)
...
inf = c(-1.791805671)                               # (inflexion point) empirical values (mean and between-day sd) [2]
bet = c(0.005352109, 0)                        # (beta) empirical values (mean and between-day sd) [2]
...
colnames(xy) = NULL
for (i in 1:iter){
  temp = rbind(xy[i,],c(rx,ry))                              # Merging fish position and receivers position
...
# iterP = ceiling(iter/P)
# RECP = array(NA,dim=c(iterP,R))
# for (i in 1:iterP){
#   RECP[i,] = colSums(REC[(1+P*(i-1)):(P*i),])
# }

#### my implementation of RECP ####
datebins = seq.POSIXt(from = as.POSIXct(start_date), to = as.POSIXct(end_date), by = P*dt)
RECP = array(0, dim = c((length(datebins)-1), R))
for(r in 2:length(datebins)){
  for(c in 1:length(unique(rd$station_name))){
    RECP[r-1,c] = length(which(indv_data$datetime >= datebins[r-1] & indv_data$datetime < datebins[r] &
                                 as.character(indv_data$station) == unique(as.character(rd$station))[c]))
  }
}

REC = matrix(0, ncol = ncol(RECP), nrow = nrow(RECP))
REC[which(RECP != 0)] = 1
...
data.jags = list(
  ...
  x0 = xy[1,1],                # Initial fish position (at tag-and-release) (longitude, m)
  y0 = xy[1,1],                # Initial fish position (at tag-and-release) (latitude, m)
